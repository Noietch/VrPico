...
public struct AppSettings: Codable, Equatable, Sendable {
    ...
    public var clientURL: URL? {
        serverURL(port: clientPort)
    }

    public var viserURL: URL? {
        serverURL(port: viserPort)
    }

    /// 先校验 host 和 port，再构造 URL。IPv6 字面量需要方括号；带 zone id 的
    /// 地址还需要把 `%` 编码成 `%25`。
    private func serverURL(port: Int) -> URL? {
    ...
    public func picoNativeWebSocketURL() -> String {
        ...
        return components.string ?? "ws://127.0.0.1:\(webxrPort)/ws?token=\(Self.nativeToken)"
    }
    ...
    public func validate() -> [ValidationIssue] {
        ...
        issues.append(contentsOf: Self.validatePort(webxrPort, field: .webxrPort, label: "WebXR 端口"))

        if webxrMode.trimmingCharacters(in: .whitespaces).isEmpty {
            issues.append(ValidationIssue(field: .webxrMode, message: "WebXR mode 不能为空"))
        }
